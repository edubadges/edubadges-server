...
urlpatterns = [
    ...
    url(r'^badgeclasses/edit/(?P<entity_id>[^/]+)$', BadgeClassDetail.as_view(), name='badgeclass_detail'),

    url(r'^badgeclasses/award-enrollments/(?P<entity_id>[^/]+)$', BatchAwardEnrollments.as_view(), name='badgeclass_award_enrollments'),
    url(r'^issuers/(?P<issuerSlug>[^/]+)/badges/(?P<badgeSlug>[^/]+)/assertions/(?P<entity_id>[^/]+)$', BadgeInstanceDetail.as_view(), name='badgeinstance_detail'),
