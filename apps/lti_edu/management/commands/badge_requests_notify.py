...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for user in staff:
            badge_classes_new_enrollments = dict((badge_class, counter) for badge_class, counter in list(badge_classes.items()) if user in badge_class.issuer.staff.all())
            badge_classes_old_enrollments = dict((badge_class, counter) for badge_class, counter in list(old_badge_classes.items()) if user in badge_class.issuer.staff.all())
