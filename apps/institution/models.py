...
class Institution(
    EntityUserProvisionmentMixin, PermissionedModelMixin, ImageUrlGetterMixin, BaseVersionedEntity, BaseAuditedModel
):
    def __str__(self):
        return self.name or ''
    ...
    award_allowed_institutions = models.ManyToManyField(
        'self', blank=True, symmetrical=False, help_text='Allow awards to this institutions'
    )
    ...
    linkedin_org_identifier = models.CharField(
        max_length=255, blank=True, null=True, default=None, help_text='LinkedIn organisation identifier'
    )
    sis_integration_enabled = models.BooleanField(
        default=False, help_text='Set to true to enable SIS integration for this institution'
    )
    micro_credentials_enabled = models.BooleanField(
        default=False, help_text='Set to true to enable Micro-credentials for this institution'
    )
    virtual_organization_allowed = models.BooleanField(
        default=True, help_text='Set to true to enable Virtual Organizations for this institution'
    )
    ob3_ssi_agent_enabled = models.BooleanField(
        default=False, help_text='Set to true to enable OB3 integration for this institution'
    )
    ...
    manage_client_id = models.CharField(
        ...
        null=True,
        default=None,
        help_text='OpenConext client_id for token introspection. The secret is stored in Manage',
    )
    ...
    grondslag_formeel = models.CharField(
        max_length=254, null=True, blank=True, choices=GRONDSLAG_CHOICES, default=GRONDSLAG_UITVOERING_OVEREENKOMST
    )
    ...
    DEFAULT_LANGUAGE_CHOICES = ((DEFAULT_LANGUAGE_DUTCH, 'nl_NL'), (DEFAULT_LANGUAGE_ENGLISH, 'en_EN'))
    default_language = models.CharField(
        max_length=254, choices=DEFAULT_LANGUAGE_CHOICES, default=DEFAULT_LANGUAGE_DUTCH
    )
    COUNTRY_DUTCH = 'NL'
    COUNTRY_ENGLISH = 'EN'
    COUNTRY_CHOICES = ((COUNTRY_DUTCH, 'NL'), (COUNTRY_ENGLISH, 'EN'))
    ...
    def get_report(self):
        ...
        return {
            ...
            'total_admins': [
                staff for staff in self.cached_staff() if staff.permissions == staff.full_permissions()
            ].__len__(),
...
class Faculty(
    ...
    def validate_unique(self, exclude=None):
        if not self.archived:
            ...
            faculty_same_name = (
                self.__class__.objects.filter(query, institution=self.institution, archived=False)
                .exclude(pk=self.pk)
                .first()
            )
