...
class Clone(object):
    ...
    def clone(self, request, queryset):
        ...
        for instance in queryset:
            ...
            reverse_relations = (
                instance._meta.get_all_related_objects() + instance._meta.get_all_related_many_to_many_objects()
            )

            # Get related instances (relation_meta is a RelatedObject instance)
            for relation_meta in reverse_relations:
                related_instances = getattr(instance, relation_meta.get_accessor_name()).all()
                ...
                for related_instance in related_instances:
                    ...
                    setattr(new_related_instance, relation_meta.field.name, new_instance)
                    new_related_instance.save()

    clone.short_description = _lazy('Clone selected %(verbose_name_plural)s and related instances')
