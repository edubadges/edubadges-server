...
edu_id_app, _ = SocialApp.objects.get_or_create(
    provider='edu_id', name='edu_id', client_id=settings.EDU_ID_CLIENT, secret=edu_id_secret
)

surf_conext_secret = settings.SURF_CONEXT_SECRET
surf_conext_app, _ = SocialApp.objects.get_or_create(
    provider='surf_conext', name='surf_conext', client_id=settings.SURF_CONEXT_CLIENT, secret=surf_conext_secret
)
...
superuser, _ = BadgeUser.objects.get_or_create(
    is_superuser=1, username=settings.SUPERUSER_NAME, email=settings.SUPERUSER_EMAIL, is_staff=1
)
