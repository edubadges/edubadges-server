...
class BadgeClassExtensionSerializer(serializers.ModelSerializer):
    ...
    def get_value(self, obj):
        ...
        extension_key = (
            json_dict["type"][1].split(":")[1].removesuffix("Extension")
        )
...
class BadgeClassDetailSerializer(serializers.ModelSerializer):
    issuer = IssuerSerializer(read_only=True)
    badgeclassextension_set = BadgeClassExtensionSerializer(
...
class TermsAgreementCreateSerializer(serializers.ModelSerializer):
    terms = serializers.SlugRelatedField(
        queryset=Terms.objects.all(), slug_field="entity_id"
    )
...
class CatalogBadgeClassSerializer(serializers.ModelSerializer):
    ...
    # Issuer fields
    issuer_name_english = serializers.CharField(
        source="issuer.name_english", read_only=True
    )
    issuer_name_dutch = serializers.CharField(
        source="issuer.name_dutch", read_only=True
    )
    issuer_entity_id = serializers.CharField(
        source="issuer.entity_id", read_only=True
    )
    issuer_image_dutch = serializers.CharField(
        source="issuer.image_dutch", read_only=True
    )
    issuer_image_english = serializers.CharField(
        source="issuer.image_english", read_only=True
    )

    # Faculty fields
    faculty_name_english = serializers.CharField(
        source="issuer.faculty.name_english", read_only=True
    )
    faculty_name_dutch = serializers.CharField(
        source="issuer.faculty.name_dutch", read_only=True
    )
    faculty_entity_id = serializers.CharField(
        source="issuer.faculty.entity_id", read_only=True
    )
    faculty_image_dutch = serializers.CharField(
        source="issuer.faculty.image_dutch", read_only=True
    )
    faculty_image_english = serializers.CharField(
        source="issuer.faculty.image_english", read_only=True
    )
    faculty_on_behalf_of = serializers.BooleanField(
        source="issuer.faculty.on_behalf_of", read_only=True
    )
    faculty_type = serializers.CharField(
        source="issuer.faculty.faculty_type", read_only=True
    )

    # Institution fields
    institution_name_english = serializers.CharField(
        source="issuer.faculty.institution.name_english", read_only=True
    )
    institution_name_dutch = serializers.CharField(
        source="issuer.faculty.institution.name_dutch", read_only=True
    )
    institution_entity_id = serializers.CharField(
        source="issuer.faculty.institution.entity_id", read_only=True
    )
    institution_image_dutch = serializers.CharField(
        source="issuer.faculty.institution.image_dutch", read_only=True
    )
    institution_image_english = serializers.CharField(
        source="issuer.faculty.institution.image_english", read_only=True
    )
    institution_type = serializers.CharField(
        source="issuer.faculty.institution.institution_type", read_only=True
    )
