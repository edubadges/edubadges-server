...
class OB3CallbackView(APIView):
    ...
    def post(self, request: Request):
        entity_id, uuid = self.__parse_body(request)
        badge_instance = self.__get_badge_instance(entity_id)

        self.__authorize(badge_instance, uuid)
    ...
    def __authorize(self, badge_instance: BadgeInstance, uuid: str):
        # Dummy to check that the badgeinstance was issued to this user
        social_account = get_social_account(uuid)
