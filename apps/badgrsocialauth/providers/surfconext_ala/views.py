...
from .provider import SurfconextAlaProvider
...
def login(request):
    ...
    # state contains the data required for the redirect after the login via SURFconext and ALA,
    ...
    data = {'client_id': _current_app.client_id,
            'redirect_uri': '%s/account/surfconext_ala/login/callback/' % settings.HTTP_ORIGIN,
            'response_type': 'code',
...
def after_terms_agreement(request, **kwargs):
    ...
    provider = SurfconextAlaProvider(request)
